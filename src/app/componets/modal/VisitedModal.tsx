...
import SelectLocationVisite from "./SelectLocationVisite";
...
const VisitedModal = () => {
  ...
  useEffect(() => {
    ...
    if (hasVisited === "true") {
      setIsVisible(false); // Don't show the modal if the cookie is set
    } else {
      setIsVisible(true); // Show modal if the cookie is not set
      Cookies.set("hasVisited", "true", { expires: 365 }); // Set cookie for next visits
    }
  }, []);
